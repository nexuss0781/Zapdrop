...
for test_name in \
  ...
  persists_sparse_resume_ranges_for_large_file_fixture; do
  run_test "" "$test_name"
done
for test_name in \
  qualifies_large_fixture_determinism_and_bounded_metadata \
...
printf '%s\n' 'Snapshot qualification passed: deterministic 512-file indexing, serialized metadata bounds, subtree reuse, and sparse journal persistence passed in default and swarm-v2 builds.'
