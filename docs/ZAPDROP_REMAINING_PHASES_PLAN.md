...
Phase 17 remains the only active phase. The next implementation action is **Phase 17’s multi-process child-routing slice**: route only the authenticated, grant-scoped opaque pieces through the live relay session and preserve direct fallback. No repair, companion, or release work will be started in the same phase. Each Phase 17 slice must be implemented, tested, documented, committed, and pushed before the next slice begins.
