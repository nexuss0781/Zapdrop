...
**Implementation status:** The snapshot foundation is implemented and has controlled qualification coverage. It provides bounded file hashing, canonical NFC Unicode path normalization, traversal and symlink rejection, deterministic directory ordering, content-addressed directory/file/piece-index objects, bounded chained piece-index pages, crash-safe job-scoped transfer journals, disk-space preflight, sparse-range recovery, and source-generation verification. The feature-gated v2 direct sender derives its file manifest from the snapshot engine, and the v2 receiver persists authenticated ranges and completion state in the journal. A deterministic 512-file fixture now verifies stable content identifiers, serialized metadata-page bounds, unchanged-subtree reuse, and sparse journal persistence; see `docs/ZAPDROP_PHASE6_SNAPSHOT_QUALIFICATION.md`.

The remaining Phase 7 work is a network paged-metadata exchange for very large trees, subtree reuse across independent network snapshots, millions-of-entry stress qualification, durable sparse writes across process termination, explicit source mutation revisions, and 4 GiB-plus physical-file testing. See `docs/ZAPDROP_PHASE7_STATUS.md` for the exact boundary of the implemented slice.
