...
Control objects use UTF-8 JSON with `camelCase` field names and exactly one JSON object per newline-delimited control frame. Binary payloads are not placed inside control JSON. Byte strings are base64url without padding unless a field explicitly states another encoding. Hashes and identifiers use lowercase hexadecimal only when the field definition says `hex`.
...
All control frames have a maximum encoded size of 1 MiB in the initial implementation. A receiver MUST cap the number of objects, recipients, children, and outstanding requests before allocating unbounded memory.
...
A root references directory nodes and file objects. A receiver may request index pages lazily instead of accepting one monolithic manifest.
...
This application channel is an implementation foundation and is not a TLS 1.3 deployment. The `swarm-v2` feature now integrates a secure hello/proof exchange into the real TCP listener and exposes an opt-in sender probe through `ZAPDROP_SWARM_V2_PROBE`. The active v1 transfer path remains the default and unchanged. The next security slice must review the construction, migrate a feature-gated direct file transfer, add explicit re-handshake/rekey orchestration, and validate payload confidentiality with packet capture and physical-LAN tests before v2 is enabled by default.
...
The secure-channel foundation adds signed ephemeral handshakes, directional key derivation, profile negotiation, bounded rekey limits, fresh job-key provisioning, snapshot-bound authenticated data, replay protection, and AEAD tests. The `swarm-v2` feature now runs a real TCP listener probe and encrypted proof exchange; the next slice should migrate one direct file transfer behind the same feature gate. Only after that exchange passes packet-capture and physical-LAN checks should the sender and receiver runtime be migrated from v1 transfer frames to v2 direct mode.
