...
This is the first bounded network metadata exchange, not a complete paged-tree protocol. A fresh v2 loopback session now resumes from a pre-seeded persisted sparse journal and verifies missing-range negotiation and final digest integrity. Atomic-save tests verify that stale temporary crash artifacts do not replace the last committed journal, while truncated or wrong-kind records fail closed. Multiple-page transport, subtree reuse across independent network snapshots, interruption after process termination, and 4 GiB-plus physical-file acceptance remain open follow-up work.
