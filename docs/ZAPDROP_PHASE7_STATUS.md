...
The implementation includes tests for canonical snapshots, Unicode normalization, traversal rejection, bounded piece and metadata pages, exact subtree-generation matching, source mutation detection, disk-space preflight, sparse missing-range recovery, and atomic journal round trips. Remaining Phase 7 qualification work is disk-full fault injection, durable sparse writes across process termination, and 4 GiB-plus physical-file testing. Authenticated network metadata-object retrieval is recorded separately in the Phase 16 status; it does not make tree/mesh forwarding or physical-LAN behavior qualified.
...
This is the first bounded network metadata exchange, not a complete paged-tree protocol. A fresh v2 loopback session now resumes from a pre-seeded persisted sparse journal and verifies missing-range negotiation and final digest integrity. Atomic-save tests verify that stale temporary crash artifacts do not replace the last committed journal, while truncated or wrong-kind records fail closed. Active recipient cancellation is also covered on a throttled v2 loopback, and untracked large objects now split into valid piece-sized missing ranges. Cross-snapshot reuse is qualified locally with both generation and content-derived object-ID matching. Phase 16 now adds bounded authenticated retrieval of directory and piece-index objects over the v2 direct session. Receiver interruption after process termination during payload writes and 4 GiB-plus physical-file acceptance remain open follow-up work.
